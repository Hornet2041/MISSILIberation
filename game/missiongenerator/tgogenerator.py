...
class ShipGroupGenerator(GroundObjectGenerator):
    ...
    def generate(self) -> None:
        super(ShipGroupGenerator, self).generate()

        if not self.game.settings.generate_fire_tasks_for_ship_groups:
            return
    ...
    def ship_group_range(self, ship_group: ShipGroup) -> int:
        ...
        for group in self.ground_object.groups:
            ...
            if vg is not None:
                for u in vg.units:
                    unit_data = vehicle_map.get(u.type)
                    if unit_data is not None:
                        threat_range = unit_data.get('threat_range')
                        if threat_range is not None and threat_range > group_range:
                            group_range = threat_range
        return group_range
